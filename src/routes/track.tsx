...
export const Route = createFileRoute("/track")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Look up the status of your Kush & Cotton order.",
      },
    ],
  }),
});
...
function TrackPage() {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setSearched(false);
    try {
      const res = await track({ data: { reference: reference.trim() } });
      setOrder((res.order as OrderResult | null) ?? null);
      setSearched(true);
    } catch (err) {
      console.error(err);
    ...
    <SiteLayout>
      <section className="px-6 md:px-8 py-16 border-b-2 border-forest">
        ...
        <p className="mt-6 max-w-xl text-forest/70">
          Enter your order reference (e.g. <strong>KC-12345</strong>) to see
          the latest status.
        </p>
      </section>
        ...
        <section className="px-6 md:px-8 py-12">
          {!order ? (
            <p className="label-mono text-forest/60">
              No order found for that reference. Double-check and try again.
            </p>
          ) : (
            <div className="border-2 border-forest max-w-3xl">
